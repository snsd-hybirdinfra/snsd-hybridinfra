...
This scenario documents distributed database failover within the database operational domain. It
...
- Define the scenario-specific database signal represented by distributed-database-failover.
...
This scenario defines the resilience scope for **Distributed Database Failover**. It focuses on maintaining operational survivability when the following capability becomes degraded, unstable, or dependent on coordinated failover behavior:

- **Primary resilience target:** database instance, query workload, replication channel, storage backend

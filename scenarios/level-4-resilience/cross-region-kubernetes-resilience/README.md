...
This scenario documents cross region kubernetes resilience within the kubernetes / cluster
...
- Define the scenario-specific kubernetes / cluster signal represented by cross-region-kubernetes-resilience.
...
This scenario defines the resilience scope for **Cross Region Kubernetes Resilience**. It focuses on maintaining operational survivability when the following capability becomes degraded, unstable, or dependent on coordinated failover behavior:

- **Primary resilience target:** Kubernetes cluster, node, pod, control plane, service object

...
| Field | Value |
|---|---|
| Scenario Name | wan-link-monitoring |
| Lifecycle Level | level-1-visibility |
| Scenario Path | scenarios/level-1-visibility/wan-link-monitoring |
| Scenario Type | Visibility / Monitoring |
| Primary Domain | Network / WAN |
| Status | draft |
...
This scenario documents wan link monitoring within the network / wan operational domain. It focuses
on WAN link, edge router, remote site, network path and demonstrates how infrastructure operations
teams can use domain-specific telemetry, lifecycle workflow design, and evidence-backed validation
to support detect and expose operational health signals before incident escalation.

---

## Objectives

- Define the scenario-specific network / wan signal represented by wan-link-monitoring.
- Identify the affected network / wan components and dependencies.
- Collect and interpret telemetry from WAN link, edge router, remote site, network path.
...
Correlate network / wan signals with related infrastructure state, dependencies, recent events, and
service impact.
...
| Component | Purpose |
|---|---|
| Wan Link | Provides context or signal source for Network / WAN operations |
| Edge Router | Provides context or signal source for Network / WAN operations |
| Remote Site | Provides context or signal source for Network / WAN operations |
| Network Path | Provides context or signal source for Network / WAN operations |
| Telemetry Source | Provides context or signal source for Network / WAN operations |
| Detection Logic | Provides context or signal source for Network / WAN operations |
| Evidence Output | Provides context or signal source for Network / WAN operations |
...
This scenario defines the visibility scope for **Wan Link Monitoring**. It focuses on collecting, organizing, and presenting operational signals so that infrastructure state can be understood before deeper correlation or recovery decisions are required.

- **Primary visibility target:** WAN link, edge router, remote site, network path
...
The primary operational risk is **undetected degradation, missing telemetry, and delayed operational awareness**. In the context of **Wan Link Monitoring**, this means the workflow must clearly separate observable symptoms, dependency context, response boundaries, and validation evidence.
...
- [Vpn Tunnel Health Monitoring](/snsd-hybridinfra/scenarios/level-1-visibility/vpn-tunnel-health-monitoring/README.md)

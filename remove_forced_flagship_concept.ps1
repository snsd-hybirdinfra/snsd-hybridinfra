...
# Remove public/internal flagship-chain dependency from related scenario generator.
$generator = @'
...
lines.append("rule: exact primary_domain only; no fallback; no forced flagship chain")
lines.append("```")
lines.append("")
...
Set-Content -Path ".\tools\content-generator\generate_related_scenarios.py" -Value $generator -Encoding UTF8

python ".\tools\content-generator\generate_related_scenarios.py"
python ".\tools\content-generator\generate_readme_from_metadata.py"

# Remove flagship wording from root README if it exists.
if (Test-Path ".\README.md") {
    $readme = Get-Content ".\README.md" -Raw
    $readme = $readme -replace '(?s)---\s*\r?\n\r?\n## Flagship Operational Chain.*?(?=\r?\n---\s*\r?\n\r?\n## Operational Capabilities)', @'
---
...
'@
    $readme = $readme -replace 'flagship scenario chain', 'scenario coverage model'
    $readme = $readme -replace 'flagship chain', 'scenario coverage model'
    $readme = $readme -replace 'Flagship', 'Representative'
    Set-Content -Path ".\README.md" -Value $readme -Encoding UTF8
}

Remove-Item ".\a.txt" -Force -ErrorAction SilentlyContinue

Write-Host "[OK] Removed forced flagship concept from generator/report/metadata/readmes where applicable."
Write-Host "[NEXT] Review:"

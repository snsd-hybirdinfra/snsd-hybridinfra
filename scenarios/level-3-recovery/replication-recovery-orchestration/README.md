...
| Field | Value |
|---|---|
| Scenario Name | replication-recovery-orchestration |
...
This scenario documents replication recovery orchestration within the database / replication
operational domain. It focuses on replication channel, source database, replica database, lag signal
...
- Define the scenario-specific database / replication signal represented by replication-recovery-orchestration.
- Identify the affected database / replication components and dependencies.
- Collect and interpret telemetry from replication channel, source database, replica database, lag signal.
...
Correlate database / replication signals with related infrastructure state, dependencies, recent
...
| Component | Purpose |
|---|---|
| Replication Channel | Provides context or signal source for Database / Replication operations |
| Source Database | Provides context or signal source for Database / Replication operations |
| Replica Database | Provides context or signal source for Database / Replication operations |
| Lag Signal | Provides context or signal source for Database / Replication operations |
| Telemetry Source | Provides context or signal source for Database / Replication operations |
| Detection Logic | Provides context or signal source for Database / Replication operations |
| Evidence Output | Provides context or signal source for Database / Replication operations |
...
This scenario defines the recovery scope for **Replication Recovery Orchestration**. It focuses on restoring the affected capability through controlled orchestration, automation execution, and validation.

- **Primary recovery target:** replication channel, source database, replica database, lag signal
...
The primary operational risk is **unvalidated automation, unsafe remediation, and incomplete service restoration**. In the context of **Replication Recovery Orchestration**, this means the workflow must clearly separate observable symptoms, dependency context, response boundaries, and validation evidence.
...
- [Resource Rebalancing Automation](/snsd-hybridinfra/scenarios/level-3-recovery/resource-rebalancing-automation/README.md)
...
This scenario contributes to the infrastructure operations portfolio by documenting database / replication workflow design, telemetry interpretation, lifecycle execution, validation criteria, and reviewable operational evidence.

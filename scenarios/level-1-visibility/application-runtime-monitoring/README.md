...
| Field | Value |
|---|---|
| Scenario Name | application-runtime-monitoring |
| Lifecycle Level | level-1-visibility |
| Scenario Path | scenarios/level-1-visibility/application-runtime-monitoring |
| Scenario Type | Visibility / Monitoring |
| Primary Domain | Application Runtime |
| Status | draft |
...
This scenario documents application runtime monitoring within the application runtime operational
domain. It focuses on application process, runtime endpoint, container or VM-hosted service and
...
- Define the scenario-specific application runtime signal represented by application-runtime-monitoring.
- Identify the affected application runtime components and dependencies.
- Collect and interpret telemetry from application process, runtime endpoint, container or VM-hosted service.
- Use health endpoint status as an operational signal for detection or validation.
- Use process uptime as an operational signal for detection or validation.
...
- Application Process
- Runtime Endpoint
- Container Or Vm-Hosted Service
- Telemetry Source
- Detection Logic
- Evidence Output
...
health endpoint status; process uptime; restart count; HTTP status; request latency; error rate; CPU
usage; memory pressure; runtime logs

---

## Correlation and Analysis

Correlate application runtime signals with related infrastructure state, dependencies, recent
...
Monitoring and visibility include health endpoint status; process uptime; restart count; HTTP
status; request latency; error rate; CPU usage; memory pressure; runtime logs.
...
| Component | Purpose |
|---|---|
| Application Process | Provides context or signal source for Application Runtime operations |
| Runtime Endpoint | Provides context or signal source for Application Runtime operations |
| Container Or Vm-Hosted Service | Provides context or signal source for Application Runtime operations |
| Telemetry Source | Provides context or signal source for Application Runtime operations |
| Detection Logic | Provides context or signal source for Application Runtime operations |
| Evidence Output | Provides context or signal source for Application Runtime operations |
...
This scenario defines the visibility scope for **Application Runtime Monitoring**. It focuses on collecting, organizing, and presenting operational signals so that infrastructure state can be understood before deeper correlation or recovery decisions are required.

- **Primary visibility target:** application process, runtime endpoint, container or VM-hosted service
...
The primary operational risk is **undetected degradation, missing telemetry, and delayed operational awareness**. In the context of **Application Runtime Monitoring**, this means the workflow must clearly separate observable symptoms, dependency context, response boundaries, and validation evidence.
